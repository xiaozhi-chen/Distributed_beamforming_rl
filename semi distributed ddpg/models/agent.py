...
class Agent(object):
    ...
    def run(self, training_on, replay_queue, learner_w_queue, update_step):
        ...
        start_state = self.env_wrapper.reset()
        rewards = []
        while training_on.value:
            ...
            while not done:
                ...
                if self.agent_type == "exploration":
                    action = action.cpu().detach().numpy() + 0.1 * torch.randn(action.shape).cpu().detach().numpy()
#                    action = self.ou_noise.get_action(action, num_steps)
                    action = np.clip(action.squeeze(0), 0, 1)
                else:
                ...
                action = normalize(action.reshape(self.env_wrapper.M, self.env_wrapper.K), p=2)
                action = torch.flatten(action)
                next_state, reward, done = self.env_wrapper.step(action)

                episode_reward += reward
                step=update_step.value
                self.logger.scalar_summary("agent/reward", reward, step)
                ...
                if done or num_steps == self.max_steps:
                    if episode_reward > max_episode_reward:
                        print("Agent: {0: 3} Episode Reward: {1: 5.4f} Update step: {2: 7}".format(self.n_agent, episode_reward, update_step.value))
