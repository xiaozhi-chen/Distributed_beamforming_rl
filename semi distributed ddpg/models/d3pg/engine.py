...
class Engine(object):
    ...
    def train(self):
        ...
        p = threading.Thread(target=sampler_worker,
        ...
        p = threading.Thread(target=learner_worker, args=(config, training_on, policy_net, target_policy_net, learner_w_queue,
                                                          batch_queue, update_step, experiment_dir))
        processes.append(p)

        # Single agent for exploitation
        p = threading.Thread(target=agent_worker,
